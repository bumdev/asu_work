        ...
        protected void btContract_Click(object sender, EventArgs e)
        {
            ...
            if (ue.Count > 0)
            {
                    ...
                    for (int i = 0; i < ue.Count; i++)
                    {
                        ...
                        if (uev.Count > 0)
                        {
                            for (int j = 0; j < uev.Count; j++)
                            {
                                ...
                                vodomer += "определению метрологических характеристик  " + vt.Description + " Ø " + vt.Diameter.ToString() + " мм  в количестве 1 шт.,  ";
                            }
                        }
                    }
                }
                ...
                if (ua.IsBudget)
                {
                    ...
                    using (DocX document = DocX.Load(Request.MapPath("~\\Templates/budget.docx")))
                    {
                        ...
                        document.ReplaceText("SUM", RuDateAndMoneyConverter.CurrencyToTxtRub(sum, true), false, System.Text.RegularExpressions.RegexOptions.IgnoreCase);
                        ...
                        document.ReplaceText("SSS", (sum).ToString("0.00") + " рос. руб", false, System.Text.RegularExpressions.RegexOptions.IgnoreCase);
                        ...
                        //VODOMER
                        document.SaveAs(Request.MapPath("~\\Templates/outbudget.docx"));
                ...
                else
                {
                    ...
                    using (DocX document = DocX.Load(Request.MapPath("~\\Templates/nonbudget.docx")))
                    {
                        //DocXExtender.ReplaceFormatedText(document, "DDD", "дата");
                        document.ReplaceText("NNN", uo.ID.ToString(), false, System.Text.RegularExpressions.RegexOptions.IgnoreCase);
                        document.ReplaceText("DATE", RuDateAndMoneyConverter.DateToTextLong(uo.DateIn) + " г.", false, System.Text.RegularExpressions.RegexOptions.IgnoreCase);
                        ...
                        //VODOMER
                        document.SaveAs(Request.MapPath("~\\Templates/outnonbudget.docx"));
